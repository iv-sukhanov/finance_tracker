...
func Test_addCategoryAction(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.senderBeh(sender)
			cmd := commandsByIDs[1]
			addCategoryAction(tc.input, tc.batch, nil, log, sender, &Client{chanID: 1}, &cmd)
...
func Test_addCategoryDescriptionAction(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.senderBeh(sender)
			cmd := commandsByIDs[1]
			addCategoryDescriptionAction(tt.input, tt.batch, service, log, sender, &Client{userGUID: tt.userGUID, chanID: 1}, &cmd)
		})
	}
}
